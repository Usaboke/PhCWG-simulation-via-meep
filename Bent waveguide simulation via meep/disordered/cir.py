...
set_disorder=0.02
disorder_type=0   # 0 for position disorder; 1 for radius disorder
...
def phc_trans(PhC = True,Zbend = True, lengthPhC = 150, decay_check=0, T_decay=500, check_dBm=50,disorder =0):
    """
    <変数の説明>
...
if __name__ == "__main__":  
    ...
    a = a0
    c_const = c_const0
    freqs_wo,psd_out_wo = phc_trans(PhC = False, Zbend=False,lengthPhC = Lwg, decay_check=dch0, T_decay=3000, check_dBm=check_dBm0,disorder=0)
    freqs_s, psd_out_s  = phc_trans(PhC = True, Zbend=False,lengthPhC = Lwg, decay_check=dch_s, T_decay=20000, check_dBm=check_dBm0,disorder=0)
    freqs_s1, psd_out_s1  = phc_trans(PhC = True, Zbend=False,lengthPhC = Lwg, decay_check=dch_s, T_decay=20000, check_dBm=check_dBm_s,disorder=set_disorder)
    freqs_s2, psd_out_s2  = phc_trans(PhC = True, Zbend=False,lengthPhC = Lwg, decay_check=dch_s, T_decay=20000, check_dBm=check_dBm_s,disorder=set_disorder)
    freqs_s3, psd_out_s3  = phc_trans(PhC = True, Zbend=False,lengthPhC = Lwg, decay_check=dch_s, T_decay=20000, check_dBm=check_dBm_s,disorder=set_disorder)
    freqs_s4, psd_out_s4  = phc_trans(PhC = True, Zbend=False,lengthPhC = Lwg, decay_check=dch_s, T_decay=20000, check_dBm=check_dBm_s,disorder=set_disorder)
    freqs_s5, psd_out_s5  = phc_trans(PhC = True, Zbend=False,lengthPhC = Lwg, decay_check=dch_s, T_decay=20000, check_dBm=check_dBm_s,disorder=set_disorder)
    freqs_s6, psd_out_s6  = phc_trans(PhC = True, Zbend=False,lengthPhC = Lwg, decay_check=dch_s, T_decay=20000, check_dBm=check_dBm_s,disorder=set_disorder)
    freqs_s7, psd_out_s7  = phc_trans(PhC = True, Zbend=False,lengthPhC = Lwg, decay_check=dch_s, T_decay=20000, check_dBm=check_dBm_s,disorder=set_disorder)
    freqs_s8, psd_out_s8  = phc_trans(PhC = True, Zbend=False,lengthPhC = Lwg, decay_check=dch_s, T_decay=20000, check_dBm=check_dBm_s,disorder=set_disorder)
    freqs_s9, psd_out_s9  = phc_trans(PhC = True, Zbend=False,lengthPhC = Lwg, decay_check=dch_s, T_decay=20000, check_dBm=check_dBm_s,disorder=set_disorder)
    freqs_s10, psd_out_s10  = phc_trans(PhC = True, Zbend=False,lengthPhC = Lwg, decay_check=dch_s, T_decay=20000, check_dBm=check_dBm_s,disorder=set_disorder)
    ...
    df["T_s1"] = np.array(psd_out_s1)/np.array(psd_out_wo)
    df["T_s2"] = np.array(psd_out_s2)/np.array(psd_out_wo)
    df["T_s3"] = np.array(psd_out_s3)/np.array(psd_out_wo)
    df["T_s4"] = np.array(psd_out_s4)/np.array(psd_out_wo)
    df["T_s5"] = np.array(psd_out_s5)/np.array(psd_out_wo)
    df["T_s6"] = np.array(psd_out_s6)/np.array(psd_out_wo)
    df["T_s7"] = np.array(psd_out_s7)/np.array(psd_out_wo)
    df["T_s8"] = np.array(psd_out_s8)/np.array(psd_out_wo)
    df["T_s9"] = np.array(psd_out_s9)/np.array(psd_out_wo)
    df["T_s10"] = np.array(psd_out_s10)/np.array(psd_out_wo)
    df["T_ave"]=df.loc[:,'T_s1': 'T_s10'].mean(axis=1)
    df.insert(2, 'T_average', df.pop('T_ave'))
    #ax=df.plot(x="freq",y="T_s")
    #df.plot(y="T_ave",ax=ax)


    df.to_csv("transmittance_S="+str(S)+"_f="+str(f_input)+"_"+str(check_dBm_s)+"dBm"+"_a="+str(a0)+"_r1="+str(a0*r)+"_r2="+str(a0*r*rbp)+"_delta="+str(set_disorder)+".csv", index=False)

    
    plt.plot(freqs, np.array(psd_out_s)/np.array(psd_out_wo))
    plt.plot(freqs, np.array(psd_out_s1)/np.array(psd_out_wo))
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("transmittance_S="+str(S)+"_f="+str(f_input)+"_"+str(check_dBm_s)+"dBm"+"_a="+str(a0)+"_r1="+str(a0*r)+"_r2="+str(a0*r*rbp)+"_delta="+str(set_disorder)+".png")
    
    time_end = time.perf_counter()
    time = time_end - time_start
    time_min=time/60
    print("The necessary time: {:.3f}min".format(time_min))

    f = open('totaltime1.txt', 'a')
    f.write("Lwg="+str(Lwg)+"_S="+str(S)+"_"+str(check_dBm_z)+"dBm_"+"The necessary time: {:.3f}min".format(time_min))
    f.close()